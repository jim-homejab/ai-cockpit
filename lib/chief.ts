...
const CHIEF_CAN_PROPOSE = [
  ...
  "- When Front is connected through Pipedream, inventory open work with search_front_conversations (no tag required — defaults to is:open). Optionally filter by tag_name, inbox_name, assignee, or participant. Follow nextCursor until hasMore is false before reporting a full count. Then use Front MCP tools to read details and PROPOSE writes (archive, assign, tag, comment, draft reply) — those stay on Ask. Do not insist the user create a 'Chief Inbox Zero' tag before inventoring.",
  "",
