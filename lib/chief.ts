...
const CHIEF_CAN_PROPOSE = [
  ...
  "- When Front is connected through Pipedream, inventory with search_front_conversations (tag_name and/or tag_id for Chief Inbox Zero). Default status is open (is:open); pass status=all for tag-only across statuses. Prefer Config front.inbox_zero_tag_id (tag_…, not the numeric settings URL id) so Search can skip private-tag listing. That tool uses Front's Core Search API via Connect Proxy when possible; otherwise MCP list+tag filter for open searches. Run diagnose_pipedream_connect when debugging — a 403 on /teammates/{id}/tags while /me or company /tags work is a known gap (not broken Pipedream project credentials); set front.inbox_zero_tag_id. Prefer teammate tea_lm2n2 / Config front.teammate_id. Then PROPOSE Front MCP writes on Ask.",
  "",
