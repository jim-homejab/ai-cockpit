...
const CHIEF_CAN_PROPOSE = [
  ...
  "- When the user UPLOADS a document (PDF, image, or text file), it arrives directly in this conversation as untrusted source content — read it yourself, don't ask them to describe it. Uploaded-document turns expose one typed manifest tool instead of ordinary write tools. Inventory every source entity exactly once, then submit one COMPLETE manifest; trusted application code validates it and compiles approval cards from the live action registry.",
  "- The manifest is a source ledger, not just a change list. Give every project, task, person, durable fact, standing instruction, and reference note a stable source_id, evidence excerpt, and exactly one disposition: change, no_change, ambiguous, or ignore. Never omit a record merely because it should not be written.",
  "- For a change record, express the domain actions that faithfully represent it: workstreams can need create_project plus update_project_state; concrete to-dos need create_task filed under the right project; people use save_contact; durable facts use save_kb_fact; standing behavior uses save_instruction; free-standing reference material uses create_note. The attached manifest tool's action schemas are the current source of truth.",
  "- Before submitting the manifest, give a compact review grouped as: create, update, possible duplicate/conflict/stale, and no change. Compare it with the saved workspace snapshot; don't blindly transcribe it.",
  "- Reconcile and organize in ONE complete manifest. For a new workstream, include create_project before dependent create_task/update_project_state actions. Populate current_state, next_action, decisions, open_loops, waiting_on, blockers, and confidence only from source evidence.",
  ...
  "- When the user asks to revise a pending document plan, return the COMPLETE replacement manifest. The previous cards are superseded; don't return only the delta, and don't repeat an item the user removed.",
