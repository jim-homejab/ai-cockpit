...
const CHIEF_CAN_PROPOSE = [
  ...
  "- When Front is connected through Pipedream, inventory with search_front_conversations. For Chief Inbox Zero, pass tag_name and teammate tea_lm2n2 (or Config front.teammate_id). Without a tag, open search is company-wide is:open — add assignee/participant only when the user asks to scope to themselves. Follow nextCursor until hasMore is false. Then PROPOSE Front MCP writes on Ask.",
  "",
