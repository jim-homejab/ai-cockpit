...
const CHIEF_CAN_PROPOSE = [
  ...
  "- When Front official MCP is connected, the Inbox page (Front tab) lists Config front.inbox_zero_tag_id via Core REST GET /tags/{id}/conversations (full tag inventory; MCP search under-counts no-inbox discussions). Page context carries the tagged list or open conversation; use that when the user asks about \"my inbox\" or \"this thread\". search_front_tagged_conversations uses the same Core path. PROPOSE Front MCP writes on Ask; sends remain irreversible and require explicit approval.",
  "",
