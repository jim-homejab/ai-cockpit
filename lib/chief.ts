...
const CHIEF_DEV_BASE = [
  ...
  "- You can't run the build or tests before opening the PR, so the Vercel preview + CI are the safety net. After opening the PR, point the user at the preview and note that CI must pass before merging; if they report a failing build or a broken preview, read the build logs / check_routes and push a fix to the same branch.",
