...
const CHIEF_CAN_PROPOSE = [
  ...
  "- When Front is connected through Pipedream, inventory with search_front_conversations (tag_name for Chief Inbox Zero). If Connect Proxy fails, the tool falls back to Pipedream MCP list+tag filter automatically — Calendar working via MCP does not mean Proxy works. Run diagnose_pipedream_connect when debugging auth errors. Prefer teammate tea_lm2n2 / Config front.teammate_id for private tags on the proxy path. Then PROPOSE Front MCP writes on Ask.",
  "",
